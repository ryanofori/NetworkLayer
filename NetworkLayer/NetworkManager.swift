...
class NetworkManager {
    ...
    func requestMethod(url: String, param: String = "", httpMethod: String = "GET", bodyValue:String = "", headerField: String = "", completion: @escaping(Data?, URLResponse? ,Error?) -> Void) {
        ...
        guard let url = URL(string: url) else { NetworkError.staus.invalidURL; return }
    ...
    func downTask(request: URLRequest) {
            let downloadTask = URLSession.shared.downloadTask(with: request) {data, response, error in
